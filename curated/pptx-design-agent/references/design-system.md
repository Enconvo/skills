...
8. [Composition Planning](#composition-planning) — **Layout types, image composition, and overlay coordination**
...
**Content-Driven (recommended for narrative/story decks):**
- Layout type follows what each slide communicates (see Layout Type Catalog)
- Title Page → Narrative → Quote → Full-Bleed Image → Comparison → Chapter Divider → ...
- Natural variety emerges from content diversity — no forced alternation needed
- Best for: fairy tales, case studies, educational content, pitches with mixed content

**Rules:**
- **Never use the same layout type for more than 3 consecutive slides.**
- **Never default to KPI Cards for non-data slides.** If the slide content is a story, quote, comparison, or visual moment — use the matching layout type instead.

### Layout Type Catalog

Each slide in a presentation should use a **layout type** matched to what it communicates. Choose from:

#### Title Page
- **Description:** Centered title + subtitle + accent line. The opening slide.
- **When to use:** First slide of every deck; section openers in long decks.
- **NOT for:** Content slides, data slides.
- **Positioning:** Title centered vertically (~2800000 EMU top), subtitle 400000 EMU below. Accent line between or below.

#### Chapter Divider
- **Description:** Large chapter number + accent line + chapter title. Visual breathing room between sections.
- **When to use:** Multi-section decks (5+ slides), topic transitions, story act breaks.
- **NOT for:** Single-section short decks, data-heavy sequences.
- **Positioning:** Chapter number large (60-80pt) at ~1800000 EMU top, accent line below, title at ~3400000 EMU. Content centered horizontally.

#### Narrative Page
- **Description:** Flowing body text with a title, optionally with a side image. NOT cards.
- **When to use:** Story content, explanations, descriptions, educational text, any slide where the content is prose.
- **NOT for:** Metric-driven content, comparisons, lists of independent items.
- **Positioning:** Title at top (200000 EMU). Body text below (~900000 EMU), width 60-70% of slide if image present (image fills remaining 30-40%), full width if no image. Body text 16-18pt with 1.35 line spacing.

#### Quote Page
- **Description:** Large centered quote with decorative quotation mark + attribution line.
- **When to use:** Famous quotes, character dialogue, testimonials, key takeaways phrased as quotes.
- **NOT for:** Regular body text, data, lists.
- **Positioning:** Large decorative `"` at ~1200000 EMU top (60-80pt, accent color, 20% opacity). Quote text centered, 24-28pt, italic, at ~2200000 EMU. Attribution right-aligned below quote, 16pt, muted color.

#### Full-Bleed Image Page
- **Description:** Image covers entire slide with minimal text overlay (title only, or title + short subtitle).
- **When to use:** Mood setting, visual transitions, cinematic moments, establishing shots.
- **NOT for:** Text-heavy content, data, detailed explanations.
- **Positioning:** Image at 0,0 covering 12192000x6858000 EMU. Text in gradient-darkened zone (top or bottom 30%).

#### KPI / Metric Cards
- **Description:** 2-4 rounded rectangle cards, each with label + big number + note.
- **When to use:** ONLY for actual numeric data — revenue, growth rates, counts, percentages, scores.
- **NOT for:** Narrative content, story elements, quotes, descriptions, non-numeric information.
- **Positioning:** Cards arranged in row at ~1200000 EMU top, equal widths with 150000-200000 EMU gaps. Use `make_kpi_card()` helper.

#### Comparison Page
- **Description:** Side-by-side columns with headers, comparing two concepts/options/characters.
- **When to use:** Pros vs cons, before vs after, two characters, two approaches, any binary comparison.
- **NOT for:** Single-topic content, more than 2 items (use grid instead).
- **Positioning:** Two columns each ~5200000 EMU wide, separated by 500000 EMU gap or thin accent divider line. Column headers in accent color. Use `make_comparison_page()` helper.

#### Timeline
- **Description:** Horizontal or vertical sequence of events/steps with connecting line.
- **When to use:** Chronological sequences, step-by-step processes, story progression, historical events.
- **NOT for:** Non-sequential content, single-point slides.
- **Positioning:** Horizontal line at vertical center (~3400000 EMU). Event nodes (circles, 200000 EMU diameter) spaced evenly along line. Labels above/below alternating.

#### Data Table
- **Description:** Structured tabular data with styled header row and alternating row colors.
- **When to use:** Multi-column data, structured comparisons with 3+ items, schedules, specs.
- **NOT for:** Prose content, single data points (use KPI cards), visual/narrative slides.
- **Positioning:** Table positioned at ~1200000 EMU top, width ~11200000 EMU, centered. Use python-pptx `add_table()`.

#### Diagram / Process
- **Description:** Connected shapes showing flow, hierarchy, or relationships. Arrows or lines between nodes.
- **When to use:** Workflows, decision trees, organizational charts, cause-and-effect, system architecture.
- **NOT for:** Linear narratives (use timeline), simple lists, prose.
- **Positioning:** Nodes as rounded rectangles or circles, connected with arrows. Center the diagram on slide. Leave 457200 EMU margins.

#### Grid / Mosaic
- **Description:** 4-9 equal tiles in a grid, each with image and/or text. Uniform sizing.
- **When to use:** Team introductions, multi-item showcases, gallery views, feature grids, character introductions.
- **NOT for:** Single-focus content, deep narrative, data analysis.
- **Positioning:** 2x2, 3x2, or 3x3 grid starting at ~1000000 EMU top. Tiles with 150000 EMU gaps. Each tile: rounded rect with image/icon + label.

---

### Image Composition Patterns

**These patterns describe WHERE the image focal point and text zones sit, used IN COMBINATION with Layout Types above.** When a layout type uses a background image, pick a composition pattern to coordinate the image generation prompt with content placement.

> Example: A **Narrative Page** layout with a side image might use the **Split Left-Right** composition. A **Full-Bleed Image Page** might use **Bleed with Gradient Fade**. A **Title Page** with a hero background uses **Center Stage** or **Top-Bottom Split**.

The following 10 patterns are for image-overlay coordination:
...
### Layout Type Matching Guide

**Decision tree: "What does this slide communicate?"**

```
What does this slide communicate?
├── Opening / title of the deck → Title Page
├── Transition between sections → Chapter Divider
├── A story, explanation, or description → Narrative Page
├── A quote, dialogue, or testimonial → Quote Page
├── A visual mood or cinematic moment → Full-Bleed Image Page
├── Numeric data, KPIs, metrics → KPI / Metric Cards
├── Two things compared side by side → Comparison Page
├── A sequence of events or steps → Timeline
├── Structured multi-column data → Data Table
├── A process, flow, or hierarchy → Diagram / Process
└── Multiple items in a gallery/grid → Grid / Mosaic
```

**Anti-pattern rules:**
- **DO NOT default to KPI Cards for non-metric content.** If the slide doesn't contain actual numbers/data, KPI Cards are the wrong layout.
- **DO NOT use the same layout type for 3+ consecutive slides.** Mix layout types to maintain visual rhythm.
- **DO NOT force narrative content into cards.** A story paragraph belongs in a Narrative Page, not split across 3 KPI cards with truncated text.
- **DO NOT use Grid/Mosaic for sequential content.** If order matters, use Timeline instead.
