...
### 3. Image Prompt Engineering Rules
...
These are the widest, most prominent images. They set the document's visual tone.
...
3. **Color harmony**: "Color palette harmonizing with [name 2-3 hex colors from active style]"
4. **Visual style matching**: Match the deck's design language (corporate photography for STYLE-01, hand-drawn for STYLE-03, pastel illustration for STYLE-04, etc.)
5. **Composition for text overlay** (if title will be overlaid on the image via a table cell): "Dark/quiet zone in [center/bottom] for white text overlay. Subject in [upper portion/edges]."
6. **Quality**: "High resolution, professional quality, clean edges suitable for document layout"

**Example for a STYLE-02 (Executive Editorial) cover:**
```
Elegant minimalist photograph of an open notebook on a warm-toned wooden desk,
soft natural lighting from the left. Warm cream and earth tones harmonizing with
#1A1A2E and #C8102E. Bottom third is dark and uncluttered for title text overlay.
No text, no words, no letters. 16:9 widescreen aspect ratio, high resolution,
editorial photography style.
```

#### B. Content Illustration Prompts

These sit inline with text. They must complement, not dominate, the surrounding content.

**Mandatory prompt components:**
1. **No text**: Same directive as above.
2. **Appropriate ratio**: "4:3 landscape" for standard inline, "1:1 square" for callout cards, "3:4 portrait" for sidebar-style.
3. **Clean background**: Match the document's page color — "Clean white background" for light styles, or "Background matching [hex color]" for colored sections.
4. **Style coherence**: Use the active style's color palette as the image's primary palette.
5. **Visual weight**: "Clean, focused composition with clear subject. Minimal surrounding detail." Content images support text — they shouldn't be busy panoramic scenes.
6. **Edge treatment**: "Clean edges, subject centered, no important detail at the frame edges" — important because images may be cropped or rounded.

**Example for a STYLE-05 (Corporate) content illustration:**
```
Minimalist isometric illustration of a data dashboard with clean geometric shapes.
Color palette: #0071E3 blue, #34C759 green, #F5F5F7 light gray. White background.
4:3 landscape aspect ratio. Clean edges, centered composition. No text, no labels.
Flat corporate design style, simple and elegant.
```

#### C. Prompt Quality Checklist (Run Before Every Image Generation)

- [ ] **No-text directive** ("No text, no words, no letters, no typography")
- [ ] **Aspect ratio** matching the image role (16:9 cover / 4:3 content / 1:1 card)
...
- [ ] **For content images:** Clean background matching page/card surface color
- [ ] **For content images:** Subject fills 60-80% of frame, edges are clean

#### D. Post-Generation AR Verification (MANDATORY)

After each image is generated, verify the actual dimensions match the intended role:
...
# Cover banner should be ~1.78 (16:9), content ~1.33 (4:3), etc.
```

**If AR doesn't match (>15% deviation):**
1. Regenerate with a stronger directive: "IMPORTANT: This image MUST be wider than tall, 16:9 widescreen ratio"
2. If regeneration fails twice, adapt — use `crop_to_aspect()` to crop the image to the target ratio before inserting
3. NEVER force-fit by specifying both width and height — use `safe_add_picture()` which auto-calculates
...
3. **Generate all needed images** (if user said yes) — use whichever AI image generation skill/MCP is available at the system level (the user may also explicitly specify one). **Browser-based tools (e.g., baoyu-danger-gemini-web, grok-image-gen) must generate one at a time, sequentially — NEVER in parallel. API-based image/video generation tools can run in parallel.** For each image:
   - **Identify the image role** (cover banner, section header, content illustration, card image, table cell)
   - **Follow the Image Prompt Engineering Rules** (Section A for covers/headers, Section B for content). Run the Prompt Quality Checklist (Section C) before every generation call.
   - **POST-GENERATION VERIFICATION (MANDATORY):** After each image, check actual pixel dimensions (Section D). If AR deviates >15% from intended role, regenerate or use `crop_to_aspect()` to fix before inserting.
