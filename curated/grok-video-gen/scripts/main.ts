...
Options:
  ...
  --imagine           Use grok.com/imagine for image gen (instead of x.com/i/grok)
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--imagine') { out.imagine = true; continue; }
 ...
 *  Chunks the text and uses insertText with random delays between chunks. */
async function humanType(cdp: CdpConnection, sessionId: string, text: string): Promise<void> {
  // Split into chunks of 3-8 characters (humans type in bursts)
  const chunks: string[] = [];
  let i = 0;
  while (i < text.length) {
  ...
  for (const chunk of chunks) {
    await cdp.send('Input.insertText', { text: chunk }, { sessionId });
    // Random delay: 30-120ms between chunks (simulates ~60-100 WPM typing)
    await sleep(Math.floor(Math.random() * 90) + 30);
  }
  // Final pause to let React state catch up
  await sleep(500);
}
...
async function uploadReferenceImages(
  ...
  // Step 2: Clear any stale uploads from previous generations
  console.error('[grok] Clearing any stale uploads...');
  
  // First: click the X button on any existing upload thumbnails
  // In the screenshot we can see a small X icon on top-left of the thumbnail
  for (let clearAttempt = 0; clearAttempt < 3; clearAttempt++) {
    const cleared = await evaluate<boolean>(cdp, sessionId, `
      (() => {
        let found = false;
        // Look for the X/close/remove button on upload thumbnails
        // These are typically small buttons with × or X near blob/data images
        const allBtns = document.querySelectorAll('button, [role="button"]');
        for (const btn of allBtns) {
          const rect = btn.getBoundingClientRect();
          // Small close buttons (< 30px) near the compose area
          if (rect.width > 0 && rect.width <= 32 && rect.height <= 32) {
            const text = (btn.textContent || '').trim();
            const label = (btn.getAttribute('aria-label') || '').toLowerCase();
            const hasSvg = btn.querySelector('svg') !== null;
            // Match: × character, "close", "remove", "delete", or small SVG icon buttons
            // that are near image previews
            if (text === '×' || text === 'x' || text === '✕' || 
                label.includes('remove') || label.includes('close') || label.includes('delete') ||
                hasSvg) {
              // Check if this button is near a thumbnail/preview image
              const parent = btn.parentElement;
              const nearby = parent ? parent.querySelector('img') : null;
              if (nearby || label.includes('remove') || label.includes('close')) {
                btn.click();
                found = true;
              }
            }
    ...
    `);
    if (cleared) {
      console.error('[grok] Cleared stale upload thumbnail');
      await sleep(1000);
    } else {
      break;
    }
  ...
  // Step 3: Get the file input's backendNodeId and set files directly
  console.error('[grok] Setting files directly on input element (no file chooser)...');
  const inputNodeId = await evaluate<boolean>(cdp, sessionId, `
    (() => {
      const inputs = document.querySelectorAll('input[type="file"]');
  ...
  // Verify the file input actually has our file (not stale state)
  const inputFileName = await evaluate<string>(cdp, sessionId, `
    (() => {
      const inputs = document.querySelectorAll('input[type="file"]');
      for (const inp of inputs) {
  ...
  `);
  const expectedName = path.basename(absPaths[0]!);
  if (inputFileName && inputFileName !== expectedName) {
    console.error(`[grok] WARNING: File input has "${inputFileName}" but expected "${expectedName}". Retrying...`);
    // Force clear and re-set
    ...
    console.error(`[grok] Files re-set on input: ${expectedName}`);
  } else {
    console.error(`[grok] ✅ File input confirmed: ${inputFileName || expectedName}`);
  }
...
async function generateVideo(cdp: CdpConnection, sessionId: string, args: CliArgs): Promise<void> {
  ...
  // Explicitly select Video mode (in case previous run set Image mode)
  console.error('[grok-video] Selecting Video mode...');
  const videoModeClicked = await evaluate<boolean>(cdp, sessionId, `
    (() => {
      const buttons = document.querySelectorAll('button');
      for (const btn of buttons) {
        const text = (btn.textContent || '').trim().toLowerCase();
        if (text === 'video' || text === '🎬 video' || text.endsWith('video')) {
          btn.click();
          return true;
        }
      }
      ...
      for (const el of spans) {
        ...
        if (text === 'Video' && el.closest('button, [role="button"], [role="tab"]')) {
          el.closest('button, [role="button"], [role="tab"]').click();
          return true;
        }
      }
      return false;
  ...
  `);
  if (videoModeClicked) {
    console.error('[grok-video] ✅ Video mode selected');
  } else {
    console.error('[grok-video] WARNING: Could not find Video mode button — may already be in Video mode');
  }
  await sleep(1000);
  ...
  console.error(`[grok-video] Typing video prompt: "${videoPrompt.slice(0, 60)}..."...`);
  await humanType(cdp, sessionId, videoPrompt);
  ...
  if (args.reference.length > 0) {
    ...
    if (!toggleClicked) {
      console.error('[grok-video] WARNING: Could not find "Make video" button. Falling back to submit button...');
      
      // FALLBACK: Click the submit/send button directly — the black circle button
      // This is what the user had to manually click when "Make video" wasn't found
      let submitFallback = false;
      for (let sAttempt = 0; sAttempt < 5; sAttempt++) {
        const clicked = await evaluate<boolean>(cdp, sessionId, `
          (() => {
            // Strategy 1: Find submit button by type or aria-label
            const submitBtns = document.querySelectorAll('button[type="submit"], button[aria-label*="send" i], button[aria-label*="submit" i], button[aria-label*="generate" i]');
            ...
            // Strategy 2: Find the dark/black circular send button near the input area
            // It's typically a button with an SVG arrow icon, near the textarea/contenteditable
            const allButtons = Array.from(document.querySelectorAll('button'));
            for (const btn of allButtons) {
              const rect = btn.getBoundingClientRect();
              // Send buttons are typically small (24-48px), circular, near bottom-right of input
              if (rect.width >= 24 && rect.width <= 64 && rect.height >= 24 && rect.height <= 64) {
                const svg = btn.querySelector('svg');
                const hasDarkBg = getComputedStyle(btn).backgroundColor.includes('0, 0, 0') || 
                                  getComputedStyle(btn).backgroundColor.includes('rgb(0') ||
                                  btn.classList.toString().includes('dark') ||
                                  btn.classList.toString().includes('black');
                const isNearInput = !!btn.closest('form') || !!btn.closest('[class*="input"]') || !!btn.closest('[class*="compose"]');
                if (svg && (hasDarkBg || isNearInput)) {
                  const key = Object.keys(btn).find(k => k.startsWith('__reactProps$'));
                  if (key && btn[key] && typeof btn[key].onClick === 'function') {
                    btn[key].onClick({ preventDefault: () => {}, stopPropagation: () => {} });
                  }
            ...
            // Strategy 3: Just find the last button that's inside a form or input container
            const formBtns = document.querySelectorAll('form button, [class*="input"] button, [class*="compose"] button, [class*="prompt"] button');
            if (formBtns.length > 0) {
              const btn = formBtns[formBtns.length - 1];
              const key = Object.keys(btn).find(k => k.startsWith('__reactProps$'));
              if (key && btn[key] && typeof btn[key].onClick === 'function') {
                btn[key].onClick({ preventDefault: () => {}, stopPropagation: () => {} });
              }
        ...
        if (clicked) {
          console.error('[grok-video] Submit button clicked as fallback!');
          submitFallback = true;
          break;
        }
        await sleep(1500);
      ...
      if (!submitFallback) {
        ...
        console.error('[grok-video] No submit button found either. Pressing Enter as last resort...');
  ...
  // Snapshot existing video URLs BEFORE waiting — so we skip stale results from previous generations
  const preExistingVideos = await evaluate<string[]>(cdp, sessionId, `
    (() => {
      const urls = [];
      ...
      for (const a of links) { if (a.href) urls.push(a.href); }
      return [...new Set(urls)];
    })()
  `);
  if (preExistingVideos.length > 0) {
    console.error(`[grok-video] Pre-existing videos to skip: ${preExistingVideos.length}`);
  }
  ...
  while (Date.now() < deadline) {
    const result = await evaluate<{ url: string | null; status: string }>(cdp, sessionId, `
      ((preExisting) => {
        const skipSet = new Set(preExisting);
        // Check for video elements with actual URLs
        const videos = document.querySelectorAll('video');
        for (const v of videos) {
          const sources = v.querySelectorAll('source');
          for (const s of sources) {
            if (s.src && s.src.startsWith('http') && s.src.includes('.mp4') && !skipSet.has(s.src)) return { url: s.src, status: 'found' };
          }
          if (v.src && v.src.startsWith('http') && v.src.includes('.mp4') && !skipSet.has(v.src)) return { url: v.src, status: 'found' };
          // Also check for assets.grok.com video URLs
          for (const s of sources) {
            if (s.src && s.src.includes('assets.grok.com') && s.src.includes('video') && !skipSet.has(s.src)) return { url: s.src, status: 'found' };
          }
          if (v.src && v.src.includes('assets.grok.com') && v.src.includes('video') && !skipSet.has(v.src)) return { url: v.src, status: 'found' };
        }
        ...
        for (const a of links) {
          const href = a.href;
          if (href && href.startsWith('http') && href.includes('.mp4') && !skipSet.has(href)) return { url: href, status: 'found' };
        }
      ...
      })(${JSON.stringify(preExistingVideos)})
    `);
...
/** Generate IMAGE via grok.com/imagine (instead of x.com/i/grok) */
async function generateImagineImage(cdp: CdpConnection, sessionId: string, args: CliArgs): Promise<void> {
  console.error('[grok-imagine] Step 1: Locating input on grok.com/imagine...');
  ...
  console.error(`[grok-imagine] Found input: ${foundSelector}`);

  // Click "Image" mode button (default may be Video)
  console.error('[grok-imagine] Selecting Image mode...');
  const imageModeClicked = await evaluate<boolean>(cdp, sessionId, `
    (() => {
      // Look for the Image/Video toggle buttons at the bottom
      const buttons = document.querySelectorAll('button');
      for (const btn of buttons) {
        const text = (btn.textContent || '').trim().toLowerCase();
        // Match "Image" button but not "Video" — it may have an icon prefix
        if (text === 'image' || text === '📷 image' || text.endsWith('image')) {
          btn.click();
          return true;
        }
      }
      // Fallback: look for elements with "Image" text near the input area bottom
      const spans = document.querySelectorAll('span, div, label');
      for (const el of spans) {
        const text = (el.textContent || '').trim();
        if (text === 'Image' && el.closest('button, [role="button"], [role="tab"]')) {
          el.closest('button, [role="button"], [role="tab"]').click();
          return true;
        }
      }
  ...
  `);
  if (imageModeClicked) {
    console.error('[grok-imagine] ✅ Image mode selected');
  } else {
    console.error('[grok-imagine] WARNING: Could not find Image mode button — may already be in Image mode');
  }
  await sleep(1000);
  ...
  const prompt = args.prompt || 'Generate an image';
  console.error(`[grok-imagine] Typing prompt: "${prompt.slice(0, 60)}..."...`);
  await humanType(cdp, sessionId, prompt);
  await sleep(2000);

  // Submit — use the same multi-strategy approach as video
  console.error('[grok-imagine] Submitting...');
  let submitted = false;

  const submitClicked = await evaluate<boolean>(cdp, sessionId, `
    (() => {
      // Strategy 1: submit/send/generate buttons
      const submitBtns = document.querySelectorAll('button[type="submit"], button[aria-label*="send" i], button[aria-label*="submit" i], button[aria-label*="generate" i]');
      if (submitBtns.length > 0) {
        const btn = submitBtns[submitBtns.length - 1];
        const key = Object.keys(btn).find(k => k.startsWith('__reactProps$'));
      ...
      // Strategy 2: dark circular button near input
      const allButtons = Array.from(document.querySelectorAll('button'));
      for (const btn of allButtons) {
        const rect = btn.getBoundingClientRect();
        if (rect.width >= 24 && rect.width <= 64 && rect.height >= 24 && rect.height <= 64) {
  ...
  if (submitClicked) {
    console.error('[grok-imagine] Submit button clicked!');
    submitted = true;
  }

  if (!submitted) {
    console.error('[grok-imagine] No submit button found, pressing Enter...');
    await cdp.send('Input.dispatchKeyEvent', { type: 'rawKeyDown', key: 'Enter', code: 'Enter', windowsVirtualKeyCode: 13, nativeVirtualKeyCode: 13 }, { sessionId });
    await sleep(50);
    await cdp.send('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Enter', code: 'Enter', windowsVirtualKeyCode: 13, nativeVirtualKeyCode: 13 }, { sessionId });
  }

  // Snapshot ALL existing image URLs BEFORE submit — so we only count NEW ones
  const preExistingImages = await evaluate<string[]>(cdp, sessionId, `
    (() => {
      const urls = [];
      const imgs = document.querySelectorAll('img');
      for (const img of imgs) {
  ...
  `);
  console.error(`[grok-imagine] Pre-existing images to skip: ${preExistingImages.length}`);

  // Wait for images to appear
  console.error('[grok-imagine] Waiting for image generation...');
  await sleep(5000);
  ...
  // Hash reference images for dedup
  for (const refPath of args.reference) {
    try {
      const refData = await fs.promises.readFile(refPath);
      refHashes.add(createHash('md5').update(refData).digest('hex'));
    } catch {}
  }

  while (Date.now() < deadline) {
    const result = await evaluate<{ urls: string[]; status: string; error: string }>(cdp, sessionId, `
      ((preExisting) => {
        const skipSet = new Set(preExisting);
        const urls = [];
        const imgs = document.querySelectorAll('img');
        for (const img of imgs) {
          const src = img.src || '';
          // Skip tiny icons, avatars, UI elements
          if (img.naturalWidth < 200 || img.naturalHeight < 200) continue;
          // Skip blob/data URLs that are just upload previews
          if (src.startsWith('blob:') || src.startsWith('data:')) continue;
          // Skip pre-existing images (avatars, sidebar history, etc.)
          if (skipSet.has(src)) continue;
          // Skip profile/avatar images by checking parent context
          const isAvatar = img.closest('[class*="avatar"], [class*="profile"], [class*="sidebar"], [class*="history"], nav, header');
          if (isAvatar) continue;
          // Only count Grok-generated images
          if (src.includes('assets.grok.com') || src.includes('pbs.twimg.com/media') || 
              src.includes('imagine-public') ||
              (src.startsWith('http') && img.naturalWidth >= 512)) {
            urls.push(src);
          }
        }
        
        // Check for generation errors / captcha / denial
        const allText = document.body.innerText || '';
        let error = '';
        if (allText.includes('rate limit') || allText.includes('Rate limit')) {
          error = 'Rate limited';
        } else if (allText.includes('try again') || allText.includes('Try again')) {
          error = 'Generation failed — try again message detected';
        } else if (allText.includes('captcha') || allText.includes('CAPTCHA') || allText.includes('verify')) {
          error = 'Captcha/verification detected';
        } else if (allText.includes('error') && allText.includes('generat')) {
        ...
        if (document.querySelector('[role="progressbar"]') ||
            ...
            document.querySelector('[class*="generating"]') ||
            document.querySelector('[class*="spinner"]')) {
          return { urls, status: 'generating', error };
        }
        ...
        if (error) {
          return { urls, status: 'error', error };
        }
        
        return { urls, status: urls.length > 0 ? 'found' : 'waiting', error: '' };
      })(${JSON.stringify(preExistingImages)})
    `);

    if (result.error) {
      console.error(`[grok-imagine] ⚠️ ${result.error}`);
    }

    if (result.status === 'error' && result.urls.length === 0) {
      throw new Error(`Image generation failed: ${result.error}. Grok may be rate-limiting or requiring captcha.`);
    }

    if (result.urls.length > 0 && result.status === 'found') {
      // Deduplicate
      imageUrls = [...new Set(result.urls)];
      console.error(`[grok-imagine] Found ${imageUrls.length} NEW image(s) (excluded ${preExistingImages.length} pre-existing).`);
      // Wait a bit more for any additional images
      await sleep(3000);
      const finalCheck = await evaluate<string[]>(cdp, sessionId, `
        ((preExisting) => {
          const skipSet = new Set(preExisting);
          const urls = [];
          const imgs = document.querySelectorAll('img');
      ...
      `);
      imageUrls = [...new Set([...imageUrls, ...finalCheck])];
      break;
    }

    if (result.status !== lastStatus) {
      lastStatus = result.status;
      console.error(`[grok-imagine] Status: ${result.status}...`);
    }
    await sleep(3000);
  }

  if (imageUrls.length === 0) {
    throw new Error(`No NEW images generated within ${args.timeout}s timeout. Grok may have silently failed (captcha, rate limit, or service denial). Pre-existing images were correctly excluded.`);
  }

  console.error(`[grok-imagine] Downloading ${imageUrls.length} image(s)...`);
  ...
  for (let i = 0; i < imageUrls.length; i++) {
    ...
    const savePath = imageUrls.length === 1 && !args.all ? outputPath : `${base}-${i + 1}${ext}`;

    try {
      // Download via browser fetch to include cookies
      const base64Data = await evaluate<string | null>(cdp, sessionId, `
        (async () => {
          try {
      ...
      if (base64Data) {
        ...
        // Check against reference hashes for dedup
        const imgHash = createHash('md5').update(buffer).digest('hex');
        if (refHashes.has(imgHash)) {
          console.error(`[grok-imagine] Skipped image ${i + 1}: duplicate of reference`);
          continue;
        }
      ...
      console.error(`[grok-imagine] Saved: ${savePath}`);
      savedPaths.push(savePath);
    } catch (e) {
      console.error(`[grok-imagine] Failed to download image ${i + 1}: ${e}`);
    }
  }
  ...
  if (args.json) {
    console.log(JSON.stringify({ prompt: args.prompt, imageUrls, savedPaths }, null, 2));
  } else {
...
async function main(): Promise<void> {
  ...
  if (existingPort) {
    ...
    const launchUrl = (args.video || args.imagine) ? GROK_IMAGINE_URL : GROK_URL;
  ...
  try {
    ...
    const targetUrl = (args.video || args.imagine) ? GROK_IMAGINE_URL : GROK_URL;
    ...
    // ===== IMAGE MODE via grok.com/imagine =====
    if (args.imagine) {
      await generateImagineImage(cdp, sessionId, args);
      await sleep(2000);
      return;
    }
    ...
    await humanType(cdp, sessionId, finalPrompt);
    await sleep(1000);
