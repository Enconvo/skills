...
7. **If a reference image card appears after submit, click the `Make video` button at the bottom-right of that submitted reference image card.**
   - This click is mandatory in I2V flows; otherwise generation may appear stuck with no progress.
8. Waits for video generation (up to 300s default — videos take longer)
9. Extracts video URL from DOM (video/source elements)
10. Downloads and saves as .mp4

## Troubleshooting

- **Stuck after prompt submit in video mode (I2V)**
  - Symptom: prompt is submitted, reference thumbnail is visible, but no render starts.
  - Fix: click **`Make video`** on the bottom-right of the submitted reference image card to trigger rendering.
