...
Build once, reuse for both orientations. Structure:

1. **VO bus** — place each scene wav with `adelay=<marker>|<marker>` (stereo), then `amix` them (`normalize=0`).
2. **Music bed** — a clean, VO-free score bed the length of the film (`assets/audio/ambient.wav`), `volume≈0.42`, **sidechain-ducked** under the VO so narration stays intelligible:
   `[bed][vobus]sidechaincompress=threshold=0.02:ratio=8:attack=15:release=350`
3. **Keypress SFX** (outro) — see below.
4. `amix` VO(vol ~1.3) + ducked bed + SFX (`normalize=0`), then `alimiter=limit=0.95–0.98`.
...
- These are **transients**: they cut through the VO even a few dB under it. The final master should still peak ≤ −0.3 dBFS (the `alimiter` holds it).
...
- **SFX landed:** `ffmpeg -ss <t0> -to <t1> -i mix.wav -af volumedetect -f null -` on the outro window and confirm the region max rises where the hits are.
