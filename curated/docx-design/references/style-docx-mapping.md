...
Concrete python-docx values for each document design style. Read [design-styles-catalog.md](design-styles-catalog.md) for full style descriptions — this file provides code-level implementation.
...
Every `STYLE_XX` dict (and any custom style) MUST conform to this schema so audit CHECK 9 and the `style_to_pal()` adapter work consistently.
...
| `fonts` | `dict` | Font specs keyed by role. **Required roles: `heading`, `body`.** Optional: `subheading`, `caption`, `pullquote`, `annotation`. Each value: `{"name": str, "size": Pt, "bold": bool, "color": RGBColor}` + optional `"italic": bool`. |
| `palette` | `dict` | Color specs keyed by role. **Required roles: `heading`, `body`, `muted`, `accent`.** Recommended: `accent2`, `table_header_bg`, `table_header_tx`, `table_alt_row`, `callout_bg`, `callout_border`, `border`. All values `#RRGGBB` hex strings. |
| `page` | `dict` | Page setup: `margins` (Inches), `line_spacing` (float). |

### Optional keys (style-specific extensions)

| Key | Used By | Purpose |
|---|---|---|
| `cover_pattern` | Most styles | One of `'classic_corporate'`, `'bold_banner'`, `'sketch_cover'`, `'editorial_split'`, etc. — drives cover-page layout. |
| `table_style` | Most styles | `'banded'`, `'minimal'`, `'zebra'`, etc. |
| `accent_rule` | Most styles | `{"color": "#RRGGBB", "width": Pt}` — horizontal rule spec. |
| `design_notes` | All styles | Free-form human-readable notes. |
| `positive` / `negative` | Data-driven styles | Semantic colors for values (growth/decline, gain/loss). |

### Hex string normalization rule

**All palette hex strings MUST include the leading `#` (e.g., `"#003DA5"` not `"003DA5"`).** Layout helpers use `lstrip("#")` as a band-aid, but inconsistency invites bugs. Normalize every custom style to `#RRGGBB` format.

### Validation

Run `validate_style_dict(style)` at load time for any custom style (user-authored dict):

```python
def validate_style_dict(style):
    """Raise ValueError if a style dict is missing required keys."""
    required = {'name', 'fonts', 'palette', 'page'}
    missing = required - set(style.keys())
    if missing:
        raise ValueError(f"Style dict missing required keys: {sorted(missing)}")

    font_required = {'heading', 'body'}
    font_missing = font_required - set(style['fonts'].keys())
    if font_missing:
        raise ValueError(f"Style fonts missing required roles: {sorted(font_missing)}")

    palette_required = {'heading', 'body', 'muted', 'accent'}
    ...
    # Normalize hex strings (add # prefix if missing, uppercase)
    for key, val in style['palette'].items():
        if isinstance(val, str) and not val.startswith('#'):
            style['palette'][key] = '#' + val
    return True
```

### Converting to `pal` dict for layout helpers

Layout helpers (`add_callout_box`, `make_kpi_row`, `add_pull_quote`) expect a different `pal` dict format — hex strings with helper-oriented keys. Use `style_to_pal(STYLE_XX)` to adapt. See [python-docx Reference → pal contract](python-docx-reference.md#palette-pal-dict-contract).
