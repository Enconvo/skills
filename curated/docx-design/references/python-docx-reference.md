...
def safe_add_picture(doc, img_path, max_width_inches=6.5, max_height_inches=9.0,
                     target_aspect_ratio=None, crop_on_mismatch=True):
    ...
    Args:
        target_aspect_ratio: If set, auto-crop the image to this ratio before
                             insertion when source AR deviates >15% from target.
                             Example: 1.78 for 16:9, 0.75 for 3:4.
        crop_on_mismatch: If True (default) and target_aspect_ratio is set,
                          crop with crop_to_aspect() when AR mismatches. If False,
                          raise ValueError on mismatch (strict mode).

    Returns the inline shape. On AR mismatch with cropping, img_path is replaced
    in-place in the reference — caller sees the cropped path via the returned shape.
    """
    final_path = img_path
    if target_aspect_ratio is not None:
        ...
        img.close()
        deviation = abs(src_ar - target_aspect_ratio) / target_aspect_ratio
        if deviation > 0.15:
            if crop_on_mismatch:
                final_path = crop_to_aspect(img_path, target_aspect_ratio, 1.0)
            else:
                raise ValueError(
                    f"Image AR mismatch: source is {src_ar:.2f}, target is "
                    f"{target_aspect_ratio:.2f} (deviation {deviation:.0%}). "
                    f"Pass crop_on_mismatch=True to auto-crop, or regenerate the image."
                )
    w, h = calc_image_size(final_path, max_width_inches, max_height_inches)
    return doc.add_picture(final_path, width=w, height=h)
...
def safe_add_picture_to_cell(cell, img_path, max_width_inches=2.0, max_height_inches=3.0,
    ...
    Same AR-safety contract as safe_add_picture(). If target_aspect_ratio is
    specified and source deviates >15%, auto-crops (unless crop_on_mismatch=False).
    """
    final_path = img_path
    if target_aspect_ratio is not None:
        img = PILImage.open(img_path)
        ...
        if deviation > 0.15:
            if crop_on_mismatch:
                ...
                raise ValueError(
                    f"Image AR mismatch in table cell: source {src_ar:.2f} vs "
                    f"target {target_aspect_ratio:.2f}. REGENERATE or enable crop."
                )
    w, h = calc_image_size(final_path, max_width_inches, max_height_inches)
    paragraph = cell.paragraphs[0]
    run = paragraph.add_run()
    return run.add_picture(final_path, width=w, height=h)
...
def crop_to_aspect(img_path, target_width, target_height, output_path=None):
    ...
    target_width and target_height can be EITHER:
      - Both absolute values (e.g., width=1920, height=1080) → ratio = w/h
      - A ratio with height=1.0 (e.g., width=1.78, height=1.0) → 16:9
...
def prepare_image(img_path, max_width_inches=6.5, max_height_inches=9.0,
                  corner_radius=25, target_aspect_ratio=None):
    ...
    Args:
        ...
        target_aspect_ratio: Target W/H ratio (e.g., 1.78 for 16:9, 0.75 for 3:4).
    ...
    # NOTE: crop_to_aspect accepts (width, height); passing (ratio, 1.0) is the
    # canonical way to request a ratio. Don't pass ratio as the first arg alone.
    if target_aspect_ratio is not None:
        processed = crop_to_aspect(processed, target_aspect_ratio, 1.0,
...
# ✅ RIGHT — Inline image, width only, height auto-calculated
doc.add_picture('path/to/image.png', width=Inches(6.5))

# ❌ WRONG — specifying both width AND height WILL DISTORT the image unless
#    the source AR exactly matches 4:3. python-docx stretches silently:
doc.add_picture('path/to/image.png', width=Inches(4), height=Inches(3))
# Only do this if you've PRE-VERIFIED the ratio matches the source image,
# OR use safe_add_picture(doc, path, target_aspect_ratio=1.33, crop_on_mismatch=True)
# which crops to the target AR before inserting.
...
def audit_image_ratios(doc, expected_ratios=None, tolerance=0.02):
    """Check all inline images for aspect ratio consistency.
    Run this after building any document with images.

    Args:
        doc: python-docx Document
        expected_ratios: Optional list of expected W/H ratios per image (in order).
                         If None, just reports ratios without flagging — use this
                         for reconnaissance. Pass the list to actually validate.
        tolerance: Allowed deviation from expected (0.02 = 2%).

    Returns:
        list of issue dicts with keys {index, placed_ratio, expected_ratio, msg}.
        Empty list means all images passed (or no expected_ratios supplied).
    """
    issues = []
    for i, shape in enumerate(doc.inline_shapes):
        w, h = shape.width, shape.height
        placed_ratio = w / h if h else 0
        print(f"  Image {i+1}: {w}x{h} EMU, ratio={placed_ratio:.3f}")
        if expected_ratios and i < len(expected_ratios):
            expected = expected_ratios[i]
            if expected and abs(placed_ratio - expected) / expected > tolerance:
                msg = (f"⚠️  Image {i+1}: distorted (placed AR={placed_ratio:.3f}, "
                       f"expected {expected:.3f}, deviation "
                       f"{abs(placed_ratio-expected)/expected:.1%})")
                print(f"    {msg}")
                issues.append({
                    'index': i, 'placed_ratio': placed_ratio,
                    'expected_ratio': expected, 'msg': msg,
                })
    return issues
```
...
All layout helpers (`add_callout_box`, `make_kpi_row`, `add_pull_quote`, etc.) accept
a `pal` dict with **hex string** values (NOT `RGBColor` objects). Required keys:

```python
pal = {
    'accent':           '#C8102E',   # primary accent color (headings, rules, highlights)
    'body':             '#333333',   # main body text color
    'muted':            '#8C8279',   # subtitle / secondary text
    'callout_bg':       '#F5F0E1',   # callout box background shading
    'callout_border':   '#C8102E',   # callout box left accent border
    'table_header_bg':  '#003DA5',   # optional — table header row fill
    'table_header_fg':  '#FFFFFF',   # optional — table header text color
    'table_alt_row':    '#F5F5F5',   # optional — alternating row fill
}
```

All hex values MUST include the leading `#` for consistency across styles. The
`STYLE_XX` dicts in `style-docx-mapping.md` use a DIFFERENT format (`RGBColor`
values, different key names). Convert with `style_to_pal()` before passing to
layout helpers:

```python
pal = style_to_pal(STYLE_02)  # returns the hex-string dict helpers expect
add_callout_box(doc, "Heads up", "Body copy here.", pal)
```
...
def style_to_pal(style_dict):
    """Adapter: convert a STYLE_XX dict (RGBColor values, style-specific keys)
    into the hex-string `pal` dict that layout helpers expect.

    Fills sensible defaults for missing optional keys so helpers never crash on
    KeyError. See the `pal` dict contract above the helpers section.
    """
    ...
    accent = h('primary', h('accent', '#C8102E'))
    body   = h('text', h('body', '#333333'))
    muted  = h('secondary', h('muted', h('tertiary', '#8C8279')))
    return {
        'accent':           accent,
        'body':             body,
        'muted':            muted,
        'callout_bg':       h('callout_bg', h('bg_alt', '#F5F0E1')),
        'callout_border':   h('callout_border', accent),
        'table_header_bg':  h('table_header_bg', accent),
        'table_header_fg':  h('table_header_fg', '#FFFFFF'),
        'table_alt_row':    h('table_alt_row', h('bg_alt', '#F5F5F5')),
    }


_REQUIRED_PAL_KEYS = ('accent', 'body', 'muted', 'callout_bg', 'callout_border')

def _validate_pal(pal, required=_REQUIRED_PAL_KEYS, caller=""):
    """Raise ValueError if `pal` is missing keys a layout helper needs."""
    missing = [k for k in required if k not in pal]
    if missing:
        raise ValueError(
            f"{caller or 'Layout helper'} requires palette keys {missing} but they "
            f"are not in the `pal` dict. Available keys: {sorted(pal.keys())}. "
            f"Use style_to_pal(STYLE_XX) to build a valid pal dict, or add the "
            f"missing keys to your custom pal."
        )
...
def add_callout_box(doc, title, body, pal, title_size=11, body_size=11):
    ...
    Requires `pal` to have: 'callout_bg', 'callout_border', 'accent', 'body'.
    Raises ValueError with a helpful message if any key is missing.
    """
    _validate_pal(pal, required=('callout_bg', 'callout_border', 'accent', 'body'),
                  caller='add_callout_box')
    p = doc.add_paragraph()
