...
1. [Word Presence Check](#word-presence-check)
...
**Microsoft Word.app is NOT guaranteed to be installed.** Many macOS users only have Pages, or no word processor at all. The .docx file is generated by python-docx regardless; only the live-edit / field-update / PDF-export steps need Word. Check before any AppleScript command.

### Detect Word

```bash
# Returns the app bundle ID if installed, exits non-zero if not.
osascript -e 'id of app "Microsoft Word"' 2>/dev/null
```
...
def word_installed():
    """Return True if Microsoft Word.app is installed on this Mac."""
...
```python
if word_installed():
    # ... open, update fields, verify, export PDF ...
    subprocess.run(['open', '-a', 'Microsoft Word', docx_path])
    print(f"✅ Opened {docx_path} in Word.")
else:
    print(f"ℹ️ Microsoft Word is not installed on this Mac.")
    print(f"📄 File saved at: {docx_path}")
    print(f"   Open it in Pages, LibreOffice, or your preferred .docx viewer.")
    print(f"   Note: field updates (TOC, page numbers) and PDF export will need to be "
          f"done manually or on a machine with Word installed.")
```

**Never fail the build because Word is missing.** The .docx file is the real deliverable — opening it, updating fields, and exporting PDF are conveniences that degrade gracefully.

**Pages compatibility:** Pages can open .docx files but does NOT execute Word's field codes (TOC, cross-references). If the user is Pages-only and the document has a TOC, tell them: "Your TOC is a placeholder; open in Word to populate it, or export via LibreOffice which also supports Word field codes."
...
## Known Quirks & Deprecated Patterns

**The following AppleScript operations are DEPRECATED — use python-docx instead. They exist in the sections below for backward-compatibility with existing scripts, but new code should NOT use them.** The reliability cost is not worth the convenience.

| Operation | Problem | Do this instead |
|---|---|---|
| Font color via AppleScript | Silently ignored ~20% of the time | python-docx `run.font.color.rgb` |
| Paragraph/cell shading | Inconsistent across Word for Mac versions | python-docx lxml `set_paragraph_shading()` / `set_cell_shading()` |
| Table cell borders | Only partial API exposure | python-docx lxml `set_cell_border_single()` |
| Advanced table formatting (merge cells, row heights) | Not fully exposed | python-docx + lxml |
| Insert picture via AppleScript | Positioning unreliable | python-docx `safe_add_picture()` |

**Reliable AppleScript operations (keep using):**
...
**Reliable live-edit use cases (AppleScript's sweet spot):**
- Reading document content (paragraphs, selections, styles)
- Plain text find/replace
- Field updates (TOC, page numbers, cross-references) — the #1 reason AppleScript exists
- PDF export / print settings
- Navigation (go to page, go to bookmark, select range)
- Opening and saving files

**Anything else — especially any visual/styling change — go through python-docx.**
