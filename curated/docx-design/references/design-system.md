...
**Many image gen tools have NO native aspect ratio parameter — AR is requested via prompt text only, and models frequently ignore it.** Every prompt must be explicit and role-aware.

#### Universal Rules (ALL image prompts must include):
- **No-text directive**: "No text, no words, no letters, no typography, no watermarks — purely visual."
- **Explicit aspect ratio**: "16:9 widescreen" / "4:3 landscape" / "1:1 square" / "3:4 portrait"
- **Color harmony**: Name 2-3 hex colors from the active style palette
- **Visual style**: "photorealistic", "watercolor", "minimal flat design", "line illustration" — must match the document's design language
- **Quality**: "High resolution, professional quality"

#### Role-Specific Rules:

**Cover banners / Section headers** (wide, prominent):
- AR: "16:9 widescreen" or "3:1 ultra-wide panoramic"
- If title text will be overlaid (via dark table cell): "Dark/quiet zone in [position] for text overlay"
- Composition: "Subject in [upper/center] portion, [lower/bottom] area fades to dark for text"
- Match the style's overall tone: editorial photography for STYLE-02, hand-drawn for STYLE-03, etc.

**Content illustrations** (inline with text):
- AR: "4:3 landscape" for standard inline, "1:1 square" for card/callout, "3:4 portrait" for sidebar
- Background: match page color — "clean white background" for light styles, or specific hex for colored sections
- Visual weight: "Clean, focused composition with clear subject. Minimal surrounding detail."
- Edge treatment: "Clean edges, subject centered, no important detail at frame edges" (images may be rounded)
- NEVER 16:9 for small inline images — that's for covers/banners only

**Table cell images** (small icons/thumbnails):
- AR: "1:1 square" almost always
- Style: "Icon-style illustration" or "simple flat design"
- Background: match cell/page color
- Subject fills 70-90% of frame (no tiny subject in vast empty space)

#### Post-Generation Verification (MANDATORY):
After generating each image, check actual pixel dimensions with PIL:
```python
from PIL import Image as PILImage
img = PILImage.open('generated.png')
w, h = img.size
actual_ar = w / h
# Cover: expect ~1.78 (16:9). Content: ~1.33 (4:3). Card: ~1.0 (1:1)
```
If AR deviates >15%: regenerate with stronger directive, or use `crop_to_aspect()` to fix before inserting.
