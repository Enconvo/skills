...
All layout helpers (`make_header_row`, `make_data_rows`, `make_totals_row`,
`make_kpi_card`, `make_cover_sheet`) accept a `pal` dict of **hex string values
WITHOUT the leading `#`** (openpyxl's `start_color` / `color` fields want the bare
6-char form). Required keys:
...
pal = {
    ...
    # Table header
    'header_fill':    '003DA5',   # REQUIRED for make_header_row
    'header_font':    'FFFFFF',   # REQUIRED for make_header_row

    # Body rows
    'alt_row_fill':   'F5F5F5',

    # KPI cards
    'kpi_card_bg':    'F8F9FA',   # optional — omit to skip card background
    ...
    # Decoration
    'border':         'CCCCCC',   # optional
}
```

`STYLE_XX` dicts in `style-xlsx-mapping.md` already store this format under
`style['palette']`, so `style_dict['palette']` can be passed directly as `pal`.
`style_to_pal()` below is a safety adapter that fills missing keys.
...
_REQUIRED_PAL_KEYS = ('heading', 'body', 'muted', 'accent',
                      'header_fill', 'header_font', 'alt_row_fill',
                      'positive', 'negative')


def _validate_pal(pal, required=_REQUIRED_PAL_KEYS, caller=""):
    """Raise ValueError if `pal` is missing keys the layout helper needs.

    openpyxl silently accepts bad/missing colors and produces visually broken
    output. Validating at entry makes the error visible and actionable.
    """
    missing = [k for k in required if k not in pal]
    if missing:
        raise ValueError(
            f"{caller or 'Layout helper'} requires palette keys {missing} but they "
            f"are not in the `pal` dict. Available: {sorted(pal.keys())}. "
            f"Use style_to_pal(STYLE_XX) to build a valid pal, or add the missing "
            f"keys to your custom pal."
        )


def style_to_pal(style_dict):
    """Adapter: extract and validate `pal` from a style dict, filling sensible
    defaults for optional keys. Safe to call even if some keys are missing.
    """
    palette = dict(style_dict.get('palette', {}))  # shallow copy

    # Fill defaults for recommended-but-sometimes-missing keys
    palette.setdefault('header_font', 'FFFFFF')
    palette.setdefault('header_fill', palette.get('accent', '003DA5'))
    palette.setdefault('alt_row_fill', 'F5F5F5')
    palette.setdefault('positive', '16A34A')
    palette.setdefault('negative', 'DC2626')
    palette.setdefault('muted', '999999')

    # Strip any leading '#' — openpyxl wants bare hex
...
def setup_styles(wb, style_dict):
    ...
    On re-registration conflict (same name, different definition) the new style
    REPLACES the old one — this ensures custom styles can override presets
    cleanly instead of being silently ignored.
    """
    for style_name, font_spec in style_dict['fonts'].items():
        ...
        try:
            ...
            # Style already registered — delete old and re-add so the new
            # definition wins. Silently passing would cause custom overrides
            # to be ignored.
            if style_name in [s.name for s in wb.named_styles] if hasattr(wb, 'named_styles') else []:
                # openpyxl doesn't expose a direct delete; skip gracefully
                # and warn the caller that the existing definition wins.
                import warnings
                warnings.warn(
                    f"NamedStyle '{style_name}' already exists; keeping existing "
                    f"definition. Create wb fresh or pick a unique name to override."
                )


def auto_width(ws, min_width=6, max_width=50, padding=2):
    """Calculate and set column widths based on content.
    openpyxl has no auto-fit — this approximates it.

    Default min_width=6 matches design-system.md ID-column minimum.
    Raise to 8 if you want wider minimums for primary data tables.
    """
...
def make_header_row(ws, row_num, headers, style_dict):
    ...
    Uses the HEADING font (not body) for visual hierarchy — headers need to
    read as distinct from body rows. Also defends against palette dicts that
    are missing 'header_font' / 'header_fill' (common in custom styles).
    """
    _validate_pal(style_dict.get('palette', {}),
                  required=('header_font', 'header_fill', 'accent'),
                  caller='make_header_row')
    pal = style_dict['palette']
    heading_font = style_dict['fonts'].get('heading') or style_dict['fonts'].get('body', {})
    for col_idx, header in enumerate(headers, 1):
        cell = ws.cell(row=row_num, column=col_idx, value=header)
        cell.font = Font(
            name=heading_font.get('name', 'Calibri'),
            size=11, bold=True,
            color=pal.get('header_font', 'FFFFFF'),
        )
        cell.fill = PatternFill(
            start_color=pal.get('header_fill', '003DA5'),
            end_color=pal.get('header_fill', '003DA5'),
            fill_type='solid',
        )
    ...
    ws.row_dimensions[row_num].height = style_dict.get('sheet_setup', {}).get('header_row_height', 28)
...
def make_chart(ws, chart_type, data_ref, cat_ref, title, anchor, palette_colors,
    ...
    palette_colors: list of bare hex strings (e.g. ['003DA5', 'C8102E']).
    Use GraphicalProperties + ColorChoice so openpyxl actually renders the fill —
    assigning a raw hex string to solidFill silently accepts but produces a
    theme-colored chart in Excel.
    """
    ...
    # Apply palette colors to series via ColorChoice (the form openpyxl actually
    # serializes into the chart XML). Assigning a raw hex string to solidFill
    # looks like it worked but produces an unstyled chart.
    for i, s in enumerate(chart.series):
        if i < len(palette_colors):
            color = palette_colors[i].lstrip('#')
