...
STYLE-AWARE: If a style is active:
  ...
  - Exception: pure black (#000000) allowed UNLESS the active style forbids it
    (e.g., STYLE-12 Retro requires dark navy #1B2838 instead — see 9g)
  - FLAG WARNING for off-palette colors
  - FLAG CRITICAL if a style explicitly forbids a color present in the workbook
```
...
For custom styles, validate structure first (9-pre), then run 9a-9g.

9-pre — CUSTOM STYLE STRUCTURE VALIDATION (always first if style is custom):
  - style dict must have keys: 'name', 'fonts', 'palette', 'sheet_setup'
  - style['fonts'] must have at least 'heading' and 'body' subkeys
  - style['palette'] must have at least 'heading', 'body', 'muted', 'accent',
    'header_fill', 'header_font', 'alt_row_fill', 'positive', 'negative'
  - All palette values must be valid 6-char hex strings (without '#')
  - style['sheet_setup'] must have 'default_row_height' and 'header_row_height'
  FLAG CRITICAL on any missing required key — custom styles must meet the contract.

Load the active style dict from references/style-xlsx-mapping.md (presets)
...
## Iterative Fix Loop (Snapshot-and-Rollback)

Each pass is bracketed by a file snapshot. If a pass INCREASES the critical
count (a regression), we revert to the pre-pass snapshot and abort rather
than compounding broken state. After MAX_PASSES, any remaining criticals are
surfaced honestly in the final report — never silently delivered.
...
MAX_PASSES = 5
snapshot_path = path + ".audit-snapshot.xlsx"
prev_critical_count = None
final_report = None
...
for pass_num in range(1, MAX_PASSES + 1):
    ...
    issues = run_all_checks(wb)  # Checks 1-10 (9 runs for any style)
    critical = [i for i in issues if i.severity == 'CRITICAL']

    # Regression guard: if criticals went UP vs last pass, revert.
    if prev_critical_count is not None and len(critical) > prev_critical_count:
        print(f"Pass {pass_num}: regression detected "
              f"({prev_critical_count} → {len(critical)} criticals). Reverting.")
        shutil.copy2(snapshot_path, path)
        final_report = {'status': 'reverted', 'remaining': prev_critical_count}
        break

    if not critical:
        print(f"Clean after {pass_num - 1} fix passes")
        final_report = {'status': 'clean', 'remaining': 0}
        break
    ...
    # Loop exhausted without break — remaining criticals must be surfaced
    wb = load_workbook(path)
    remaining = [i for i in run_all_checks(wb) if i.severity == 'CRITICAL']
    final_report = {'status': 'max_passes_exceeded',
                    'remaining': len(remaining),
                    'issues': remaining}
    print(f"WARNING: {len(remaining)} critical issues remain after {MAX_PASSES} passes")
...
# NEVER silently deliver a workbook with unresolved criticals.
# Surface final_report to the user.
```

**Reporting rules:**
- `status == 'clean'` → safe to deliver
- `status == 'reverted'` → tell the user the last pass regressed; deliver the last-good state with the `remaining` criticals listed
- `status == 'max_passes_exceeded'` → tell the user the loop hit its cap; list remaining criticals verbatim so the user can decide to continue manually
