...
## Excel Presence Check (MANDATORY before any AppleScript call)

AppleScript IPC only works if Microsoft Excel is installed. Before invoking
any `tell application "Microsoft Excel" ...` block, verify presence. On a
system without Excel, fail gracefully and deliver the raw .xlsx with a note.

```python
import subprocess

def excel_is_installed() -> bool:
    """Return True iff Microsoft Excel is installed and responds to AppleScript."""
    try:
...
# Usage at start of finalization
if excel_is_installed():
    # Run the close-reopen + recalc + auto-fit chain
    run_applescript_finalization(xlsx_path)
else:
    print(
        f"[WARN] Microsoft Excel not detected. Skipping AppleScript finalization. "
        f"Open {xlsx_path} manually in Excel to recalculate formulas and auto-fit "
        f"columns."
    )
```

**Never call `osascript` without this guard** during finalization. A missing
Excel should produce a delivery with a clear note, not a confusing
AppleScript error the user has to interpret.

## Known Quirks (and what to prefer)

| Issue | Status | Prefer |
|-------|--------|--------|
| Stale display after openpyxl write | Known — close+reopen required | Close without saving + reopen |
| `calculate` can timeout on large workbooks | Known | Set calculation to manual first, calculate specific range |
| `value of cell` returns display text, not underlying value | Known | Use `formula of cell` for formulas, `value of cell` for data |
| Font color setting via AppleScript | **UNRELIABLE — deprecated for styling** | Do all color changes via openpyxl, not AppleScript |
| Large workbook reads can timeout | Known | Read sheet by sheet, range by range |
| Row height / column width unit mismatch | Known — AppleScript uses points, openpyxl uses character units | Keep all width/height authoring in openpyxl; AppleScript only for auto-fit |
| Chart style changes via AppleScript | **UNRELIABLE — deprecated** | Build/edit all charts in openpyxl |
| Conditional formatting via AppleScript | **UNRELIABLE — deprecated** | Conditional formatting lives in openpyxl only |

**Deprecation banner:** rows marked UNRELIABLE have been observed to silently
no-op or corrupt on recent Excel for Mac versions. Do NOT route around them —
use openpyxl for those operations and reopen the file.
