...
## Style Dict Schema (Contract)

Every style — preset AND custom — MUST conform to the dict structure below. The
audit (CHECK 9) validates against this schema before running any style-compliance
sub-checks, so a malformed custom style fails loudly rather than silently.

```python
STYLE_SCHEMA = {
    "name": str,                        # Required — human-readable name
    "fonts": {                          # Required — at minimum heading + body
        "heading": {"name": str, "size": int, "bold": bool, "color": "HEXHEX"},
        "body":    {"name": str, "size": int, "bold": bool, "color": "HEXHEX"},
        # Optional: subheading, metric, caption — same shape
    },
    "palette": {                        # Required — flat hex strings (no '#')
        "heading": "HEXHEX",
        "body": "HEXHEX",
        "muted": "HEXHEX",
        "accent": "HEXHEX",
        ...
        "positive": "HEXHEX",
        "negative": "HEXHEX",
        # Optional: kpi_card_bg, chart_colors (list)
    },
    "sheet_setup": {                    # Required
        "default_row_height": int,
        "header_row_height": int,
        "default_col_width": int,
        "freeze_row": int,
    },
    "cover_pattern": str,               # Required — e.g. "classic_corporate"
    "table_style": str,                 # Required — e.g. "banded", "minimal"
    "accent_border": {"color": "HEXHEX", "style": str},  # Required
    "conditional_format": {             # Required
        "positive": "HEXHEX",
        "negative": "HEXHEX",
        "bar_color": "HEXHEX",
    },
    "design_notes": str,                # Optional, human-facing
}
```

### validate_style_dict()

Use this before running any audit or layout-helper call against a custom style:

```python
_REQUIRED_TOP_KEYS = (
    "name", "fonts", "palette", "sheet_setup",
    "cover_pattern", "table_style", "accent_border", "conditional_format",
)
_REQUIRED_FONT_KEYS   = ("heading", "body")
_REQUIRED_PAL_KEYS    = (
...
_REQUIRED_SHEET_KEYS  = (
    "default_row_height", "header_row_height",
    "default_col_width", "freeze_row",
)
_REQUIRED_CF_KEYS     = ("positive", "negative", "bar_color")


def validate_style_dict(style, name_hint="custom"):
    """Raise ValueError describing the FIRST missing key. Never silent."""
    def _check(d, required, label):
        missing = [k for k in required if k not in d]
        if missing:
            raise ValueError(
                f"Style '{name_hint}' missing {label} keys: {missing}. "
                f"See STYLE_SCHEMA in style-xlsx-mapping.md."
            )

    _check(style, _REQUIRED_TOP_KEYS, "top-level")
    _check(style["fonts"], _REQUIRED_FONT_KEYS, "fonts")
    _check(style["palette"], _REQUIRED_PAL_KEYS, "palette")
    _check(style["sheet_setup"], _REQUIRED_SHEET_KEYS, "sheet_setup")
    _check(style["conditional_format"], _REQUIRED_CF_KEYS, "conditional_format")

    # Hex validation (no leading '#')
    import re
    ...
    for key, val in style["palette"].items():
        ...
        if not isinstance(val, str) or not hex_re.match(val):
            raise ValueError(
                f"Style '{name_hint}' palette['{key}']='{val}' "
                f"is not a 6-char hex string without '#'."
            )
    return True
```

**When to call:** at the start of any build script that uses a non-preset style,
and inside CHECK 9-pre. Presets are validated at import time so they fail fast.
