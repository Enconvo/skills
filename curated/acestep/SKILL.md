...
1. Checks prerequisites (git, uv — auto-installs uv if missing)
2. Clones repo (or pulls if already exists)
3. Installs Python dependencies (`uv sync`)
4. Downloads models (VAE, DiT turbo, LM model — skips if already present)
5. Configures skill to point at local server
6. Starts the server (auto-detects macOS/Linux, MLX/PyTorch)
