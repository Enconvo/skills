...
show_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  setup       First-time local setup (clone, deps, models, start)"
...
ACESTEP_DEFAULT_DIR="$HOME/ACE-Step-1.5"
ACESTEP_REPO_URL="https://github.com/ace-step/ACE-Step-1.5.git"

cmd_setup() {
    local install_dir="$ACESTEP_DEFAULT_DIR"
    local start_server=true
    local lm_model="acestep-5Hz-lm-0.6B"
    local skip_models=false
    local server_port=7860
    ...
    echo -e "${BOLD}============================================${NC}"
    echo -e "${BOLD}  ACE-Step 1.5 — Local Setup${NC}"
    echo -e "${BOLD}============================================${NC}"
    echo ""
    ...
    if ! command -v uv &> /dev/null; then
        ...
        if ! command -v uv &> /dev/null; then
            echo -e "${RED}Error: Failed to install uv. Install manually:${NC}"
            echo "  curl -LsSf https://astral.sh/uv/install.sh | sh"
            echo "  or: brew install uv"
            exit 1
        fi
    fi
    echo -e "  uv: $(uv --version 2>&1 | head -1)"
    ...
    if [[ "$(uname)" == "Darwin" && "$arch" == "arm64" ]]; then
        ...
        echo -e "  Platform: macOS Apple Silicon (MLX backend)"
    elif [[ "$(uname)" == "Darwin" ]]; then
        platform="macos-intel"
        echo -e "  Platform: macOS Intel"
    elif [[ "$(uname)" == "Linux" ]]; then
        platform="linux"
        echo -e "  Platform: Linux"
        if command -v nvidia-smi &> /dev/null; then
            echo -e "  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1)"
        fi
    fi
    echo ""

    # --- Step 2: Clone repo ---
    echo -e "${CYAN}[2/6] Setting up repository...${NC}"
    if [[ -d "$install_dir/.git" ]]; then
        echo -e "  Repository already exists at: ${install_dir}"
        echo -e "  Pulling latest changes..."
        (cd "$install_dir" && git pull --ff-only origin main 2>/dev/null) || true
    else
        echo -e "  Cloning ACE-Step 1.5 to: ${install_dir}"
        git clone "$ACESTEP_REPO_URL" "$install_dir" 2>&1 | tail -2
    fi
    echo ""

    # --- Step 3: Install dependencies ---
    echo -e "${CYAN}[3/6] Installing Python dependencies (uv sync)...${NC}"
    echo -e "  This may take a few minutes on first run..."
    (cd "$install_dir" && uv sync 2>&1 | tail -3)
    echo ""

    # --- Step 4: Download models ---
    if [[ "$skip_models" == "false" ]]; then
        echo -e "${CYAN}[4/6] Downloading models...${NC}"

        # Check which models already exist
        local checkpoints_dir="$install_dir/checkpoints"
        local models_to_download=()

        # Always need VAE + turbo DiT
        if [[ ! -d "$checkpoints_dir/vae" ]] || [[ ! -d "$checkpoints_dir/acestep-v15-turbo" ]]; then
            echo -e "  Downloading core models (VAE + DiT turbo)..."
            (cd "$install_dir" && uv run acestep-download 2>&1 | tail -3)
        else
            echo -e "  Core models (VAE + DiT) already present."
        fi

        # Download requested LM model
        if [[ -d "$checkpoints_dir/$lm_model" ]]; then
            # Check if it's complete (has safetensors files)
            local has_weights=$(find "$checkpoints_dir/$lm_model" -name "*.safetensors" 2>/dev/null | head -1)
            if [[ -n "$has_weights" ]]; then
                echo -e "  LM model ${lm_model} already downloaded."
            else
                echo -e "  LM model ${lm_model} incomplete, downloading weights..."
                (cd "$install_dir" && uv run python -c "
from huggingface_hub import snapshot_download
snapshot_download('ACE-Step/${lm_model}', local_dir='checkpoints/${lm_model}', ignore_patterns=['*.md'])
print('Done')
" 2>&1 | tail -3)
            fi
        else
            echo -e "  Downloading LM model: ${lm_model}..."
            (cd "$install_dir" && uv run acestep-download --model "$lm_model" 2>&1 | tail -3)
            # Verify weights were downloaded
            local has_weights=$(find "$checkpoints_dir/$lm_model" -name "*.safetensors" 2>/dev/null | head -1)
            if [[ -z "$has_weights" ]]; then
                echo -e "  ${YELLOW}Downloading full weights from HuggingFace...${NC}"
                (cd "$install_dir" && uv run python -c "
from huggingface_hub import snapshot_download
snapshot_download('ACE-Step/${lm_model}', local_dir='checkpoints/${lm_model}', ignore_patterns=['*.md'])
print('Done')
" 2>&1 | tail -3)
        ...
        echo ""
        echo -e "  ${GREEN}Models in checkpoints/:${NC}"
        ...
        echo -e "${CYAN}[4/6] Skipping model download (--skip-models)${NC}"
    fi
    echo ""

    # --- Step 5: Configure skill ---
    echo -e "${CYAN}[5/6] Configuring skill...${NC}"

    # Auto-configure the skill to point at local server
    local api_url="http://127.0.0.1:${server_port}"
    _ensure_config
    # Update config for local use
    local tmp_config=$(mktemp)
    jq --arg url "$api_url" '.api_url = $url | .api_mode = "native"' "$CONFIG_FILE" > "$tmp_config" && mv "$tmp_config" "$CONFIG_FILE"
    echo -e "  API URL: ${api_url}"
    echo -e "  API mode: native"
    echo ""

    # --- Step 6: Start server ---
    if [[ "$start_server" == "true" ]]; then
        echo -e "${CYAN}[6/6] Starting ACE-Step server...${NC}"

        local server_log="/tmp/acestep_server.log"
        local server_cmd=""

        if [[ "$platform" == "macos-apple-silicon" ]]; then
            echo -e "  Backend: MLX (Apple Silicon native)"
            ACESTEP_LM_BACKEND=mlx TOKENIZERS_PARALLELISM=false \
                nohup uv run --directory "$install_dir" acestep \
                --port "$server_port" \
        ...
        echo ""
        echo -e "  ${YELLOW}Server is loading models (may take 1-2 minutes)...${NC}"
        echo -e "  Check status with: $0 health"
    else
        echo -e "${CYAN}[6/6] Skipping server start (--no-start)${NC}"
        echo ""
        echo -e "  To start manually:"
        if [[ "$platform" == "macos-apple-silicon" ]]; then
            echo -e "    cd $install_dir && ./start_gradio_ui_macos.sh"
        else
            echo -e "    cd $install_dir && ./start_gradio_ui.sh"
        fi
    fi
    ...
    echo -e "  Gradio UI:    http://127.0.0.1:${server_port}"
    echo -e "  API:          http://127.0.0.1:${server_port}"
    echo -e "  LM model:     ${lm_model}"
    echo ""
    echo -e "  Generate music:"
    echo -e "    $0 generate -c \"Pop, female vocal\" -l \"[Verse] Hello world\" --duration 30"
    echo ""
}
