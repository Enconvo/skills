...
def has_inline_buttons(msg):
    """Check if message has inline keyboard buttons (not reply keyboard)."""
    if not msg.reply_markup or not hasattr(msg.reply_markup, 'rows'):
        return False
    # Check that buttons are inline (have .data) vs reply keyboard (no .data)
    try:
        rows = msg.reply_markup.rows
        if not rows:
            return False
        btn = rows[0].buttons[0]
        return hasattr(btn, 'data') and btn.data is not None
    except (IndexError, AttributeError):
        return False


def has_reply_keyboard(msg):
    """Check if message has a reply keyboard (text buttons, not inline)."""
    if not msg.reply_markup or not hasattr(msg.reply_markup, 'rows'):
        return False
    try:
        rows = msg.reply_markup.rows
        if not rows:
            return False
        btn = rows[0].buttons[0]
        return not (hasattr(btn, 'data') and btn.data is not None)
    except (IndexError, AttributeError):
        return False
...
async def cmd_set(args):
    ...
    async with client:
        ...
        if args.setting in ("inline", "joingroups", "privacy", "inline-feedback"):
            if msgs and args.value:
                if has_inline_buttons(msgs[0]):
                    result = await click_button(client, msgs[0], args.value)
                    if result:
                        msgs = result
                elif has_reply_keyboard(msgs[0]) or msgs[0].reply_markup:
                    # Reply keyboard — just send the value as text
