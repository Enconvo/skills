# Historical OpenClaw release notes

Preserved from the pre-2026.8.1 skill. These are historical observations, not current operating instructions. Use matching installed CLI/schema and current SKILL.md guidance. In particular, never manually clear session stores or edit device authorization as a routine repair.

## What's New in v2026.8.1

Verified against the installed CLI and packaged changelog on 2026-08-31. See the [official release notes](https://docs.openclaw.ai/releases/2026.8.1) and [update guide](https://docs.openclaw.ai/install/updating) for context.

- **State migration:** Shared state and agent sessions increasingly live in SQLite. Doctor imports legacy state and records migration receipts before retirement. Use supported inspection/backup/restore commands; JSON files may now be archival rather than authoritative.
- **Update recovery:** `update repair` finalizes Doctor/plugin convergence without restarting. Plugin install/update/enable may require explicit, artifact-bound capability consent; review it separately from `--yes`.
- **Model routes:** Doctor migrates shipped `codex/*` and `openai-codex/*` IDs to `openai/*`, preserving Codex runtime intent and surfacing conflicts. `modelPolicy.allow` is separate from model aliases/parameter settings.
- **Plugin changes:** More bundled providers are external packages; configured missing providers should be recovered through supported repair. Bundled OpenProse and `/prose` were removed; migration preserves existing `.prose` data. Custom plugins need SDK compatibility checks. Announced September 1 SDK removals are not yet proof a symbol is absent in this build.
- **Defaults worth reviewing:** Sessions no longer reset daily/on idle unless configured. Default concurrency is CPU-aware, with explicit values preserved. Private same-agent recall, dreaming, and self-learning behavior changed; preserve explicit disable/propose policies. Heartbeat monitoring is migrated into automation infrastructure, with owner delivery only when routable. Do not create new schedules while merely upgrading.
- **CLI coverage:** The regenerated references cover 73 current entry points including local Browser/OpenClam plugins and the updated Codex plugin's `codex` CLI. New/discovered surfaces include `automations`, `connect`, `database`, `file-transfer`, `fleet`, `resume`, `telemetry`, `triage`, `users`, and `worker`. Removed root entries from the prior snapshot include `commitments` and `crestodian`; verify nested help rather than assuming replacements.
- **Operational improvements:** Conversation search/branches, background-work recovery, structured questions, progress cards, interactive dashboards, backup restore, headless agent execution, and opt-in telemetry/remote features expanded. Do not enable optional remote access or telemetry during routine maintenance.
