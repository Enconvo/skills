"""R1-10: RSI + ATR Trailing Stop — The original strategy, now on daily."""
import sys, os
...
class RSIATRTrailing(Strategy):
    ...
    def next(self):
        ...
        atr_val = self.atr[-1]
        if np.isnan(atr_val) or np.isnan(self.rsi[-1]):
            return
...
stats = bt.run()
print(f"R1-10 RSI+ATR Trail | Return: {stats['Return [%]']:.1f}% | Sharpe: {stats['Sharpe Ratio']:.2f} | Sortino: {stats['Sortino Ratio']:.2f} | MaxDD: {stats['Max. Drawdown [%]']:.1f}% | WinRate: {stats['Win Rate [%]']:.1f}% | Trades: {stats['# Trades']} | SQN: {stats['SQN']:.2f}")
