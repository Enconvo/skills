"""R2-03: RSI Wider Entry + ATR Trailing Exit — More trades than R1-02's tight RSI."""
import sys, os
...
class RSIWiderATR(Strategy):
    ...
    def next(self):
        ...
        atr_val = self.atr[-1]
        if np.isnan(self.rsi[-1]) or np.isnan(atr_val):
            return
...
bt = Backtest(data, RSIWiderATR, cash=100_000, commission=0.001, exclusive_orders=True)
stats = bt.run()
print(f"R2-03 RSI Wide+ATR | Return: {stats['Return [%]']:.1f}% | Sharpe: {stats['Sharpe Ratio']:.2f} | Sortino: {stats['Sortino Ratio']:.2f} | MaxDD: {stats['Max. Drawdown [%]']:.1f}% | WinRate: {stats['Win Rate [%]']:.1f}% | Trades: {stats['# Trades']} | SQN: {stats['SQN']:.2f}")
